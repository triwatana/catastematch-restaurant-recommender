...
def load_data(path="data/processed/CA_Restaurants_cleaned.csv"):
    ...
    # Fill required columns safely
    df["name"] = df.get("name", "").fillna("")
    df["categories"] = df.get("categories", "").fillna("")
    df["price"] = df.get("price", 0).fillna(0).astype(str)
    df["attributes"] = df.get("attributes", "").fillna("")
    df["stars"] = df.get("stars", 0).fillna(0)
    df["review_count"] = df.get("review_count", 0).fillna(0)
    df["address"] = df.get("address", "").fillna("")
